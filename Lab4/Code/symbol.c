...
void handleProgram(GTN* program) {
	...
	char *rname = "read";
	readfunc->u.function.funcname = rname;

	readfunc->u.function.paramnum = 0;
	readfunc->u.function.param = NULL;
	Type readret = (Type) malloc(sizeof(Type_));
	readret->kind = BASIC;
	readret->u.basic = 0;
	readfunc->u.function.rettype = readret;
	readfunc->u.function.isdefined = 1;
	insertSymbol("read", readfunc);
	...
	char * wname = "write";
	writefunc->u.function.funcname = wname;
	writefunc->u.function.paramnum = 1;
	Type paramtype = (Type) malloc(sizeof(Type_));
	...
	field_var->name = NULL;
	field_var->type = paramtype;
	field_var->tail = NULL;

	writefunc->u.function.rettype = NULL;
	writefunc->u.function.param = field_var;

	writefunc->u.function.isdefined = 1;
	insertSymbol("write", writefunc);
		...
		if(!strcmp(treeNode->childrenList[0]->unitName, "Exp")) {
			...
			Sym sym = searchSymbol(treeNode->childrenList[0]->lexValue);
				if(sym == NULL) {
					...
					//printf("kind %d\n", arguments->type->kind );
					//printf("kind2 %d\n", sym->type->u.function.param->type->kind);
